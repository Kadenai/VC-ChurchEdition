...
def transcribe(input_file, model_name='large-v3', project_folder='tmp', language='pt'):
    print(i18n("Iniciando transcrição de {}...").format(input_file))
    ...
    if os.path.exists(srt_file) and os.path.exists(tsv_file) and os.path.exists(json_file):
        requested_language = str(language or "pt").strip().lower().replace("pt-br", "pt").replace("pt_br", "pt")
        existing_language = ""
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                existing_data = json.load(f)
            existing_language = str(existing_data.get("language", "")).strip().lower()
        except Exception:
            existing_language = ""

        if existing_language and existing_language != requested_language:
            print(f"Transcrição existente em '{existing_language}' difere de '{requested_language}'. Recriando...")
            try:
                os.remove(srt_file)
                os.remove(tsv_file)
                os.remove(json_file)
            except Exception as e:
                print(f"Aviso ao limpar arquivos antigos de transcrição: {e}")
        else:
    ...
    language_code = str(language or "pt").strip().lower()
    ...
    try:
        ...
        # Default language from CLI/parameter
        ...
        if potential_subs:
            ...
            if parsed and len(parsed) > 0:
                ...
                # Forçar idioma padrão para alinhamento
                detected_language = language_code
        ...
        if alignment_only and start_segments:
            ...
            try:
                ...
                if "task" in str(te):
                    print("Aviso: esta versão do WhisperX não aceita 'task' em transcribe(); usando fallback sem 'task'.")
                    result = model.transcribe(
                        audio,
                        batch_size=2,
                        chunk_size=10,
