    ...
    for i, segment in enumerate(segments):
        ...
        if not os.path.exists(source_video):
            ...
            if os.path.exists(source_video_final):
                ...
                # Tenta padrões novos com título: 000_Titulo*.mp4
                idx_prefix = f"{str(i).zfill(3)}_"
                candidates = []
                for folder in [burned_folder, "final", "cuts", "."]:
                    if not os.path.exists(folder):
                        continue
                    for fname in os.listdir(folder):
                        if fname.lower().endswith(".mp4") and fname.startswith(idx_prefix):
                ...
                if candidates:
                    ...
                    print(i18n("Warning: Could not find video file for segment {} ({})").format(i+1, title))
                    continue
        ...
        try:
            ...
            print(i18n("Error copying video for segment {}: {}").format(i, e))
            continue
        ...
        try:
            ...
            print(i18n("Error saving JSON for segment {}: {}").format(i, e))
            
        processed_count += 1
        print(i18n("Saved: {}").format(clean_title))

    print(i18n("Organization completed. {} virals saved in '{}' folder.").format(processed_count, virals_root))
